...
handleApprovePaidLeaveRequestRouter.post(
    '/',
        ...
        try {
            ...
            if (
                ...
                const missingFields = [
                    ...
                    (!payload?.leader_id || payload?.leader_id.trim() === '') &&
                        'leader_id',
                ...
                return errorResponse(
                    res,
                    400,
                    `bad request with missing field: ${missingFields}`
                );
            }

            const update_result = await update_approve_leave_request_controller(
                payload
            );
            if (!update_result?.success) {
                return errorResponse(res, 400, `${update_result?.message}`);
            }
