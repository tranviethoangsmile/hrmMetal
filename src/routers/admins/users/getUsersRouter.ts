...
import { get_all_users_of_position_for_admin_controller } from '../../../controllers';
...
getUsersRouter.post('/', async (req: Request, res: Response) => {
    try {
        ...
        const users = await get_all_users_of_position_for_admin_controller(
            adminPosition
        );
        if (!users.success) {
            return errorResponse(res, 200, users.message || 'users not found');
        }
