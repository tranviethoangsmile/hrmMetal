...
isConfirmPaidLeaveFromAdmin.post('/', async (req: Request, res: Response) => {
    try {
        ...
        if (!data?.user_id || !data?.id || !data?.admin_id) {
            ...
            return errorResponse(
                res,
                400,
                `bad request missing: ${missingFields}`
            );
        }

        const result =
            await update_confirm_from_admin_paid_leave_request_controller(data);
        if (!result?.success) {
            return errorResponse(
                res,
                200,
                `update faild with message: ${result?.message}`
            );
        }
